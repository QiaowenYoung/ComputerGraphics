...
var colorsl = [1.0, 0.0, 0.0, 1.0], colorsr = [0.0, 0.0, 1.0, 1.0];
var Txl = [], Tyl = [];
var Txr = [], Tyr = [];
var countl = 0;
var countr = 0;
 ...
 * none
 * use:
 * click on the toggle to set the view from side
 */
function setSideView(gl) {
    ...
    // Set the matrix to be used for to set the camera view
    var viewMatrix = new Matrix4();
    viewMatrix.setLookAt(0, -1, 0.75, 0, 0, 0, 0, 0, 1);
    // Set the view matrix
    gl.uniformMatrix4fv(u_ViewMatrix, false, viewMatrix.elements);
    ...
    for (var i = 0; i < countl; i++) {
        ...
        gl.uniform4f(u_FragColorl, colorsl[0], colorsl[1], colorsl[2], colorsl[3]);
        // Draw the lines
    ...
    for (var i = 0; i < countr; i++) {
        ...
        // Pass the color of a point to u_FragColor variable
        gl.uniform4f(u_FragColorr, colorsr[0], colorsr[1], colorsr[2], colorsr[3]);
        // Draw the lines
 ...
 * none
 * use:
 * click toggle to set the view from top
 */
 ...
 * none
 * use:
 * draw the defaut view, from top
 * 
 * I was considering deleting the draw part from this click function, for I have included drawing in setTopView and setSideView.
 * Later I found the behavior of canvas becoming strange. Maybe it's because if I don't draw in click function, there will be
 * delay in data transfer. I'm not quite sure, though:-)
 */
